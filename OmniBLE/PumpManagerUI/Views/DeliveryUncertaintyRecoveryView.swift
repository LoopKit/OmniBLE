...
struct DeliveryUncertaintyRecoveryView: View {
    ...
    var body: some View {
        GuidePage(content: {
            Text(String(format: LocalizedString("%1$@ has been unable to communicate with the pod on your body since %2$@.\n\nWithout communication with the pod, the app cannot resolve information about your active insulin or the insulin being delivered by the Pod. If you cannot resolve communication with this pod, you should monitor your glucose closely for the next 6 or more hours, as there may or may not be insulin actively working in your body that %3$@ cannot display.\n\nThe app will keep trying to restore communication. If you decide to give up, the Deactive Pod action will probably fail and you can discard the pod and start a new one. If it succeeds, you should have waited longer.", comment: "Format string for main text of delivery uncertainty recovery page. (1: app name)(2: date of command)(3: app name)"), self.model.appName, self.uncertaintyDateLocalizedString, self.model.appName))
                .padding([.top, .bottom])
        }) {
            VStack {
